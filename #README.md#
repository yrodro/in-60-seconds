...
First, we can exchange two rows. In this case, we exchange rows 1 & 3.
(exchange system and matrix)

Next, we can multiply a row by a scalar. In this case, let's
multiply row 1 by -2.
((-2) x row 1, system and matrix)

Finally, we can add two rows together. In this case, we add rows 1 & 3,
and substitute the result where the old row 3 values were.
(substitute)
...
Usually we combine the last two steps into one, so as to not
modify the first row. So, start again with the original matrix,
and swap the 1st and 3rd rows.
(repeats)
Then, since we want the 2nd row to begin with 0, we add -3
times the 1st row.
(replace in row 2)

Continuing in this fashion, we can simplify the system by
only looking at matrix entries; that is, without writing equations.

Multiply 1st row by -2 and add it to 3rd row in order to have
it start with 0
(1st column now reads 1 0 0)

The 1st column now reads 1,0,0. Next, we want to simplify the
second column. You already know (from math 171) that we need
to replace the 11 with 1; what we call a 'pivot'. Do we divide
by 11?
(ugly 2nd row)

NAH! Very often we can avoid fractions by playing smart
(return to previous matrix)

What if we subtract that 5 twice from the 11? We would get the
1 we wanted. So, multiply 3rd row by -2 and add it to the 2nd row.
(result)

Looks much cleaner! Let's use the new pivot to clear the 2nd
column. You know the drill.
(2nd column clean in 2 steps)

Multiply the 2nd pivot by 3 and add it to row 1.
